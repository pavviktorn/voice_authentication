...
def digital_augment(srcFace, mask_path, resized, loc='all', mode='minor'):

    src_img, tar_img, spatial_transform = st_generator(srcFace, mode)

    # read mask
    raw_hullMask = convex_hull(mask_path, loc, resized)    # size (h, w, c) mask of face convex hull
    # mask deformation
...
def transform_params(img, mode='minor'):
    ...
    hr, wr = int(round(uh*h)), int(round(uw*w))   # resize

    vmin, vmax =  -ampt, ampt
    vh, vw = random.uniform(vmin, vmax), random.uniform(vmin, vmax)
    th, tw = int(round(vh*h)), int(round(vw*w))   # translate
...
def convex_hull(mask_path, loc, resized):
    ...
    if resized:
        resize_ratio = 2
        rh, rw = mask.shape[:2]
        mask = cv2.resize(mask, (rw//resize_ratio, rh//resize_ratio), interpolation=cv2.INTER_LINEAR)
