...
def resize_aug(images, landmark=None, face_bbox=None):
    ...
    if landmark is None:
        return resize_images, None, None

    ld = np.zeros(landmark.shape).astype(int)
    for i in range(len(landmark)):
        ld[i] = [
            landmark[i][0]/resize_ratio, landmark[i][1]/resize_ratio
        ]

    fb = np.zeros(face_bbox.shape).astype((int))
    for i in range(len(face_bbox)):
        fb[i] = face_bbox[i]/resize_ratio

    return resize_images, ld, fb
