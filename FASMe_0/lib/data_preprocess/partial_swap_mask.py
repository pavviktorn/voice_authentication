...
def _sliding_bbox(mask, h, w):
    ...
    try:
        ...
        res = [0] * 4 # zzzzz due to the so small wrong face_bbox
        return res
    return res
...
def cut_face(images, landmark, alpha=1.2):
    ...
    for img in images:
        ...
        y01 = min(y0 + h, img.shape[0])
        y00 = max(y0, 0)
        x01 = min(x0 + w, img.shape[1])
        x00 = max(x0, 0)
        if (y01 - y00) <= 0 or (x01 - x00) <= 0:
            # print('error : cut_face size error')
...
def generate_partial_swap_mask(
    ...
    h_ratio, w_ratio = np.array(dssim.shape[:2]) / 256
