...
def update_learning_rate(epoch, cur_step, warmup_steps, warmup_epoch):
    lr = None
    initial_lr = 0.000001
    final_lr = 0.0001
    if cur_step < warmup_steps and epoch < warmup_epoch:
        lr = initial_lr + (final_lr - initial_lr) * (cur_step / warmup_steps)
    elif epoch < 5:
        lr = 0.0001
    elif epoch < 7:
        lr = 0.00005
