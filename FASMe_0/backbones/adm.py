...
class ADM_EndBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, kernel_size=3, stride=1):
        ...
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = nn.Conv2d(planes, planes, kernel_size=1, stride=2) #zzzz

        self.downsample = nn.Conv2d(
            inplanes, planes, kernel_size=kernel_size, stride=2 #zzzz
        )
...
class Multi_scale_Detection_Module(nn.Module):
    ...
    def __init__(
            self, inplanes, class_num=3,  #zzzz
...
class Artifact_Detection_Module(nn.Module):
    ...
    def __init__(
            self, inplanes, blocks=1, class_num=3,  #zzzz
            width_hight_ratios=2, extra_layers=None,
    ):
